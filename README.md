...
# todo service
docker stop todo-service 2>/dev/null; docker rm todo-service 2>/dev/null; docker build --no-cache -t todo-service . && docker run -d --name todo-service --network micro-net --env-file .env -p 5009:5009 todo-service
...
Audio Service
==============
docker stop audio-service 2>/dev/null; docker rm audio-service 2>/dev/null; docker build --no-cache -t audio-converter-service . && docker run -d --name audio-converter-service --network micro-net -p 5003:5003 audio-converter-service
...
docker tag audio-cutter-service:latest manoharshetty507/audio-cutter-service:latest && docker push manoharshetty507/audio-cutter-service:latest
